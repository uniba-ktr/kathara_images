FROM unibaktr/alpine AS builder
ARG JOBS=2
ARG PROFILE=default
ARG TAG=11.0.8-11.41.23
ARG JAVA_PATH=/usr/lib/jvm/java-17-openjdk

RUN apk add --no-cache ca-certificates zip python3 git bzip2 curl unzip openjdk17-jdk
...
RUN apk add --no-cache openssh-server openjdk17-jre
